-- +goose Up
-- IMPORTANT: Create your schema first and set search_path when running migrations
-- Example:
--   CREATE SCHEMA IF NOT EXISTS your_schema_name;
--   goose -dir migrations postgres "...connection... search_path=your_schema_name" up
--
-- The Makefile uses DB_SCHEMA environment variable (defaults to "workflow")
...
COMMENT ON TABLE workflow_run IS 'Durable inbox for async workflow runs';
COMMENT ON COLUMN workflow_run.id IS 'Primary key UUID';
COMMENT ON COLUMN workflow_run.type IS 'Versioned workflow type with dotted notation (e.g. billing.invoice.v1)';
COMMENT ON COLUMN workflow_run.status IS 'Current status: pending, leased, succeeded, failed, cancelled';
COMMENT ON COLUMN workflow_run.priority IS 'Lower values execute first (default: 100)';
COMMENT ON COLUMN workflow_run.run_at IS 'Earliest time to execute (for scheduling/retry backoff)';
COMMENT ON COLUMN workflow_run.idempotency_key IS 'Optional key for deduplication';
COMMENT ON COLUMN workflow_run.attempt IS 'Number of execution attempts (0-indexed)';
COMMENT ON COLUMN workflow_run.leased_by IS 'Worker ID that currently holds the lease';
COMMENT ON COLUMN workflow_run.lease_until IS 'Lease expiration timestamp';
COMMENT ON COLUMN workflow_run.deleted_at IS 'Soft delete timestamp (NULL = active)';
