...
CREATE INDEX IF NOT EXISTS idx_workflow_registry_deleted_at
    ON workflow.workflow_registry(deleted_at)
    WHERE deleted_at IS NULL;

COMMENT ON TABLE workflow.workflow_registry IS 'Registry mapping workflow names to execution runtimes';
COMMENT ON COLUMN workflow.workflow_registry.id IS 'Primary key UUID';
...
COMMENT ON COLUMN workflow.workflow_registry.deleted_at IS 'Soft delete timestamp (NULL = active)';
